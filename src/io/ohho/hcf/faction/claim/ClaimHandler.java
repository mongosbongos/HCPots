...
import io.ohho.hcf.faction.type.WildernessFaction;
...
public class ClaimHandler
{
  ...
  public static final ItemStack CLAIM_WAND = new ItemBuilder(Material.DIAMOND_HOE).displayName(ChatColor.GREEN.toString() + "Claim Wand").lore(new String[] { ChatColor.BLUE + "Left or Right Click " + ChatColor.GREEN + "a Block" + ChatColor.BLUE + " to:", ChatColor.GRAY + "Set the first and second position of ", ChatColor.GRAY + "your Claim selection.", "", ChatColor.BLUE + "Right Click " + ChatColor.GREEN + "the Air" + ChatColor.BLUE + " to:", ChatColor.GRAY + "Clear your current Claim selection.", "", ChatColor.BLUE + "Shift " + ChatColor.BLUE + "Left Click " + ChatColor.GREEN + "the Air or a Block" + ChatColor.BLUE + " to:", ChatColor.GRAY + "Purchase your current Claim selection." }).build();
    ...
    for (int x = locX - 5; x < locX + 5; x++) {
      for (int z = locZ - 5; z < locZ + 5; z++)
      {
        ...
        if (((factionAtNew instanceof RoadFaction)) || (
          ...
          (!playerFaction.equals(factionAtNew)) && ((factionAtNew instanceof ClaimableFaction))))
        {
  ...
  public boolean tryPurchasing(Player player, Claim claim)
  {
    ...
    playerFaction.broadcast(ChatColor.GOLD + player.getName() + ChatColor.GREEN + " claimed land for your faction at " + ChatColor.GOLD + '(' + center.getBlockX() + ", " + center.getBlockZ() + ')' + ChatColor.GREEN + '.', new UUID[] { player.getUniqueId() });
    return true;
  }
}
